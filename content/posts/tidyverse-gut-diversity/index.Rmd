...
Alpha and beta diversity are ecological metrics for exploratory microbiome analyses. The former measures the taxonomic variance _within_ a sample, while the latter measures how samples vary _between_ one another. 

I'm going to use some actual, anonymized 16S taxonomic data. It's a composite dataset of 222 anti-TNFa therapy naive autoimmune patients presenting with any combination of Crohn's disease (IBD), ankylosing spondyloarthritis (SpA), or acute anterior uveitis (AAU), pooled with a subset of healthy control samples from the [recently published](https://academic.oup.com/cardiovascres/advance-article-abstract/doi/10.1093/cvr/cvaa128/5831292?redirectedFrom=fulltext) Collect study. We'll be working with the genus-level rarefied data as relative abundances.
...
I've opted to for `geom_boxplot()` with a `geom_point()` overlay to get a better idea of comparison sizes. This is a perfectly nice plot that already confirms an important hypothesis about this data: patients suffering from immune-mediated inflammatory diseases have lower microbial diversity than controls with the proximity of the inflammation to the gut microbiota likely playing a role.
...
The [beta diversity](https://en.wikipedia.org/wiki/Beta_diversity) is a measure of species diversity between different ecosystems, here different patient microbiota. The Bray-Curtis dissimilarity is a semimetric statistic bounded between 0 and 1 (0 = same species in same abundances, 1 = samples do not share any species) quantifying the pairwise compositional overlap between samples.
