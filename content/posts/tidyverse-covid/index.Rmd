...
Then, let's load the complete longitudinal dataset, which defaults to a tibble, and select the testing counts that will be relevant for our analysis: positive, negative, death, and total.
...
The COVID Tracking Project has data on 56 states and territories in total, but since we'd have to go look up the populations and full names of those additional four, we will ignore them for now. We'll rename our column with the full state names as `state.name`, and then filter out states for which we didn't have a full name, i.e. the extra four.
