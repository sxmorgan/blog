...
plot.data <- testing.data %>%
    arrange(desc(tests.std)) %>%
    #mutate(ordered = paste0(date,',',std.metric))
    mutate(ordered = 1:n())

plot.data %>%
    select(c('tests.std','ordered'))
```
...
Finally, we'll order our `std.metric` variable and change the names to be a bit more intuitive.
...
testing.plot <- plot.data %>%
    #filter(!is.na(full_name) & full_name != 'District Of Columbia') %>%
    ggplot(aes(x = ordered, y = std.value)) +
    geom_point(aes(color = std.metric, shape = std.metric)) +
    # label every week on the major x axis and draw a line for each day
    # scale_x_date(
    #              date_breaks = '3 weeks',
    #              #date_minor_breaks = '1 day',
    #              date_labels = '%d.%m') +
    # add custom y-axis breaks
    # scale_y_continuous(breaks = seq(300, y.axis.max, 300), 
    #                    minor_breaks = seq(100, y.axis.max, 100)) +
    # my personal favorite color palette
    scale_color_futurama() +
    # make a grid; toggle scale = free on and off for different y-axes
    facet_wrap(~ full_name, ncol = 4, scale = 'free_x') +
    scale_x_discrete(labels = as.character(plot.data$date)) +
    # add title, remove x+y axis labels and legends
    labs(title = 'COVID-19 Testing and Prognosis in Most Populous US States',
