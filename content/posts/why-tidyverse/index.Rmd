...
So what does tidy data refer to, exactly?
...
The [gapminder data](https://www.gapminder.org/data/), which contains several per-country longitudinal metrics, by default displays this data in an untidy format. There are three variables in the table pictured below: country name (rownames), year (column names), and inflation-adjusted GDP per capita (the values). 
...
Now it looks like our image above. Let's pretend that `gapminder.wide` is our raw data and we need to tidy it. We'll use `pivot_longer()` this time, telling it to ignore the country (`cols = -country`), and gather the column names into a variable called 'year', and the corresponding values into a variable called 'gpdPercap'. We didn't have to tell `pivot_wider()` to ignore any variables/columns, because our starting `gapminder.long` data only had three variables.
...
There are other functions in `tidyr`, but these are the most important. 
...
Since these are the _most common_ data manipulations, they have [base R equivalents](https://tavareshugo.github.io/data_carpentry_extras/base-r_tidyverse_equivalents/base-r_tidyverse_equivalents.html) just like much of the `dplyr` package. To be totally honest, aside from `subset()` (equivalent to `filter()`) and `sort()`/`order()` (equivalent to `arrange()`), I never used any of these. `subset()` has the exact same syntax as `filter()`, and I'm not going to argue that there are specific advantages to using `dplyr` if you're already a pro at base R manipulations. If you're not, I found `dplyr` incredibly intuitive and easy to learn where I had been unable to do so with base R, partially because of the tidyverse's wide array of resources. 

<!-- Especially at the beginning but even to this day, I study the `dplyr` [cheatsheet](https://github.com/rstudio/cheatsheets/blob/master/data-transformation.pdf) that gives an amazing overview of tibble/data.frame manipulations. -->
