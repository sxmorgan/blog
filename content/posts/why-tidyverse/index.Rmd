...
Then, let's transform it to a [tibble](https://tibble.tidyverse.org/) and see if you can spot the differences.
...
The intent is to **make code more readable,** almost like a recipe. Indeed, the whole idea of _The Grammar of Graphics_ was to break a data visualization down into raw ingredients, which is reflected in the `ggplot2` implementation that combines these ingredients stepwise. The pipe however comes from the [magrittr](https://magrittr.tidyverse.org/) package and it functions by 'feeding' the product of a function or calculation forward to another one. 
